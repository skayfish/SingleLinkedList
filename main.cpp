...
public:
    ...
    Iterator InsertAfter(ConstIterator pos, const Type& value) {
        Node* rhs_node = pos.node_->next_node;
        Node* new_node = new Node(value, rhs_node);
...
private:
    ...
    void Initialise(const Conteiner& conteiner) {
        assert(size_ == 0 && head_.next_node == nullptr);
        SingleLinkedList<Type> tmp;
        Node* cur = &tmp.head_;
        try {
            for (const auto& element : conteiner) {
                (*cur).next_node = new Node(element, nullptr);
                cur = (*cur).next_node;
                tmp.size_++;
            }
...
// Эти функции тестируют работу SingleLinkedList
void Test1() {
